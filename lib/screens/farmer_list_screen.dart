...
class _FarmerListScreenState extends State<FarmerListScreen> {
  ...
  Widget build(BuildContext context) {
    // Accept optional category/subcategory filter from navigation args
    ...
    // Determine stream based on filters
    Stream<List<FarmerModel>> stream;
    if (navCategory != null && navSubcategory != null) {
      stream = fs.getFarmersByCategoryAndSub(navCategory, navSubcategory);
    } else if (navCategory != null) {
      stream = fs.getFarmersByCategory(navCategory);
    } else if (_searchQuery.isNotEmpty) {
      stream = fs.searchFarmers(_searchQuery);
    } else {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<FarmerModel>>(
        ...
        builder: (_, snap) {
          ...
          List<FarmerModel> farmers = snap.data ?? [];

          // Apply category filter (if set via popup, not nav)
          if (_filterCategory != null && !hasNavFilter) {
            farmers =
                farmers.where((f) => f.category == _filterCategory).toList();
          }
